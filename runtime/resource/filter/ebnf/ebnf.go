...
import (
	...
	"github.com/cloudwan/goten-sdk/runtime/resource/filter/ebnf/internal"
)
...
//	alpha = "a"…"z" | "A"…"Z" | "_" .
//	number = "0"…"9" .
func New(grammar string, options ...Option) lexer.Definition {
	// Parse grammar.
...
nextToken:
	for {
		...
		return token, &lexer.Error{Msg: fmt.Sprintf("no match found for %c", rn), Pos: pos}
	}
}
...
func (e *ebnfLexer) match(name string, expr internal.Expression, out *bytes.Buffer) (bool, error) { // nolint: gocyclo, unparam
	...
	case *internal.Token:
		return true, &lexer.Error{Msg: "internal.Token should not occur", Pos: e.r.Pos()}
...
func (e *ebnfLexerDefinition) Lex(filename string, r io.Reader) (lexer.Lexer, error) {
	if filename == "" {
...
func (e *ebnfLexerDefinition) Symbols() map[string]lexer.TokenType {
	return e.symbols
}
...
func validate(grammar internal.Grammar, expr internal.Expression) error { // nolint: gocyclo
	...
	case *internal.Name:
		if grammar.Index[n.String] == nil {
			return &lexer.Error{Msg: fmt.Sprintf("unknown production %q", n.String), Pos: lexer.Position(n.Pos())}
		}
		return nil
	...
	case *internal.Range:
		if utf8.RuneCountInString(n.Begin.String) != 1 {
			return &lexer.Error{Msg: fmt.Sprintf("start of range must be a single rune"), Pos: lexer.Position(n.Pos())}
		}
		if utf8.RuneCountInString(n.End.String) != 1 {
			return &lexer.Error{Msg: fmt.Sprintf("end of range must be a single rune"), Pos: lexer.Position(n.Pos())}
		}
		return nil
	...
	return &lexer.Error{Msg: fmt.Sprintf("unknown EBNF expression %T", expr), Pos: lexer.Position(expr.Pos())}
}
