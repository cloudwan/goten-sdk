		...
		for i := 0; i < fields.Len(); i++ {
			...
			if fieldDesc.IsList() || fieldDesc.IsMap() {
				...
				if fieldDesc.ContainingOneof() != nil {
					// ensure it exists
					protoMsg.ProtoReflect().Set(fieldDesc, protoMsg.ProtoReflect().NewField(fieldDesc))
					oneOf := fieldDesc.ContainingOneof()
					oneOfFieldValue := reflect.ValueOf(protoMsg).Elem().FieldByName(strcase.ToCamel(string(oneOf.Name()))).Interface()
					fieldType, _ = reflect.TypeOf(oneOfFieldValue).Elem().FieldByName(strcase.ToCamel(string(fieldDesc.Name())))
				} else {
